...
list
from "wiki/Post"
where date = "<% tp.date.now("YYYY-MM-DD") %> day"
```
~~~
`<% tp.date.now("YYYY-MM-DD") %>` 는 `현재년도-월-일`로 표현한다(작성일 기준으로 `2022-07-06` 이다). 
...
```
---
date: <% tp.date.now("YYYY-MM-DD") %> day
---
```
...
### 테스트 결과
테스트 결과는 [[Obsidian 데일리 노트 테스트 - 작성한 글 목록]]에서 확인할 수 있다.